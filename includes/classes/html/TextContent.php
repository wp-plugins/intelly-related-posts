    ...
    public function write(IRP_HTMLContext $context) {
        ...
        if($context->isUncuttable() || trim($this->body)=='') {
            ...
            if(defined('IRP_DEBUG_BLOCK') && IRP_DEBUG_BLOCK) {
                $context->write(sprintf('<sup style="color:red;">&nbsp;{W=%s,T=%s/%s,L=%s}</sup>'
                    , $context->currentWords
                    , $context->wordsThreshold, $irp->Options->getRewriteThreshold()
                    , $context->lastBoxWords));
            }
        } else {
            ...
            for($i=0; $i<count($text); $i++) {
                ...
                if(trim($line)!='') {
                    if (defined('IRP_DEBUG_BLOCK') && IRP_DEBUG_BLOCK) {
                        $context->write(sprintf('<sup style="color:#008000">&nbsp;{W=%s,T=%s/%s,L=%s}</sup>'
                            , $context->currentWords
                            , $context->wordsThreshold, $irp->Options->getRewriteThreshold()
                            , $context->lastBoxWords));
                    }
